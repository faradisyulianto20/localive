...
import { getPotensiDesa, updatePotensiDesa } from '../../../server/functions/profil'
...
function PotensiDesaEdit() {
  ...
  useEffect(() => {
    getPotensiDesa()
      .then((items) => {
        const item = items.find((i: PotensiDesaItem) => i.id === id)
        if (!item) {
          setErrors({ form: 'Potensi Desa tidak ditemukan' })
          return
        }
        setForm({
          image: item.image ?? '',
  ...
  return (
    <div className="max-w-2xl">
      <Card className="shadow-none border-[#EAEAEC] rounded-2xl">
        <CardHeader>
          <CardTitle className="text-lg font-semibold text-[#111214]">Edit Potensi Desa</CardTitle>
        </CardHeader>
