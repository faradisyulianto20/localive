...
function UMKMList() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['umkm'],
    queryFn: () => api.get<{ data: UmkmItem[] }>('/umkm'),
  })

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/umkm/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['umkm'] }),
  })

  const publishMutation = useMutation({
    mutationFn: (id: number) => api.post(`/umkm/${id}/publish`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['umkm'] }),
  })

  const unpublishMutation = useMutation({
    mutationFn: (id: number) => api.post(`/umkm/${id}/unpublish`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['umkm'] }),
  })
  ...
  return (
    <div>
      ...
      {isError && (
        <div className="mt-10 text-center text-sm text-red-600">Gagal memuat data.</div>
      )}
                ...
                <tr key={item.id} className="border-b border-[#EAEAEC] hover:bg-[#F7F7F8]">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      ...
                      <Link to="/admin/umkm/$id/edit" params={{ id: String(item.id) }}>
